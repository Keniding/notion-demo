...
worker.tool("createProject", {
  ...
  description: "Creates a new project with a name, description, status and priority.",
  schema: j.object({
  ...
  execute: ({ name, description, status, priority, owner, due_date }) => {
...
worker.tool("logBlocker", {
  ...
  schema: j.object({
    ...
    severity:     j.enum("low", "medium", "high", "critical")
                   .describe("How critical is this blocker."),
  }),
  execute: ({ title, description, affects, reported_by, severity }) => {
    const id = `blocker_${Date.now()}`;
    return JSON.stringify({
      id,
      title,
      description,
  ...
  execute: ({ team_name, members }) => {
    const report = members.map((m) => ({
      ...
      completion_rate: m.tasks_assigned > 0
        ? Math.round((m.tasks_done / m.tasks_assigned) * 100)
        : 0,
      status:
        m.tasks_overdue > 2                        ? "overloaded" :
        m.tasks_done / m.tasks_assigned >= 0.8     ? "performing" :
                                                     "on_track",
    }));

    const avg_completion = Math.round(
      report.reduce((acc, m) => acc + m.completion_rate, 0) / report.length
    );
    ...
    return JSON.stringify({
      ...
      members: report,
      generated_at: new Date().toISOString(),
      message: `Team "${team_name}" report generated — avg completion: ${avg_completion}%.`,
    });
  },
});